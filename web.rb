...
get '/' do
  ...
  @use_color = Color.find_by_id(cookie["color"].to_i).to_json if cookie["color"]
  @use_html = HtmlTemplate.find_by_id(cookie["html_template"].to_i).to_json if cookie["html_template"]
  if cookie["text_template"]
    text = TextTemplate.find_by_id(cookie["text_template"])
    (1..5).each {|i| text["col#{i}_text"] = nl2br(text["col#{i}_text"]) }
    @use_text = text.to_json
  end
  @use_shop = Shop.find_by_id(cookie["shop"].to_i).to_json if cookie["shop"]
